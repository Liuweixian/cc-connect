...
func init() {
	core.RegisterAgent("codely", New)
}

// Agent drives the Codely CLI in headless mode using -p --output-format stream-json.
// Codely is a Qwen fork of Gemini CLI with OpenAI-compatible API support.
//
// Modes (maps to Codely CLI approval flags):
...
type Agent struct {
	...
	model      string
	mode       string
	cmd        string // CLI binary name, default "codely"
...
func New(opts map[string]any) (core.Agent, error) {
	...
	if _, err := exec.LookPath(cmd); err != nil {
		return nil, fmt.Errorf("codely: %q CLI not found in PATH", cmd)
	}
...
func (a *Agent) Name() string { return "codely" }
...
func (a *Agent) StartSession(ctx context.Context, sessionID string) (core.AgentSession, error) {
	...
	return newCodelySession(ctx, cmd, workDir, model, mode, sessionID, extraEnv)
}

// ListSessions reads sessions from ~/.codely-cli/tmp/<project_hash>/chats/.
func (a *Agent) ListSessions(_ context.Context) ([]core.AgentSessionInfo, error) {
	return listCodelySessions(a.workDir)
}
...
func (a *Agent) SetMode(mode string) {
	...
	slog.Info("codely: mode changed", "mode", a.mode)
}
...
func (a *Agent) ProjectMemoryFile() string {
	...
	return filepath.Join(absDir, "CODELY.md")
}
...
func (a *Agent) GlobalMemoryFile() string {
	...
	return filepath.Join(homeDir, ".codely-cli", "CODELY.md")
}
...
func (a *Agent) SetActiveProvider(name string) bool {
	...
	for i, p := range a.providers {
		if p.Name == name {
			a.activeIdx = i
			slog.Info("codely: provider switched", "provider", name)
			return true
		}
	}
	return false
...
func (a *Agent) providerEnvLocked() []string {
	...
	p := a.providers[a.activeIdx]
	var env []string
	// Codely uses OpenAI-compatible API
	if p.APIKey != "" {
...
func listCodelySessions(workDir string) ([]core.AgentSessionInfo, error) {
	...
	projName := codelyProjectHash(workDir)
